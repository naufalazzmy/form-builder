<template>
    <div>
        <div class="flex items-center justify-between mt-3">
            <h2 class="font-semibold mb-2">Image Element</h2>
        ...
        <input @input="update('src', element.src)" v-model="element.src" class="w-full border p-1 mb-4" />

        <label class="block mb-1">Width</label>
        <input v-model="element.width" placeholder="%, px, pt, ect.." @input="update('width', element.width)"
            class="w-full border p-1 mb-4" />

        <label class="block mb-1">Height</label>
        <input v-model="element.height" placeholder="%, px, pt, ect.." @input="update('height', element.height)"
            class="w-full border p-1 mb-4" />
